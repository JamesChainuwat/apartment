<?php 
    ...
    $monthConv = new DateTime($month);
                                        ...
                                        $englishMonth = $monthConv->format('F');
                                        $year = $monthConv->format('Y');
                                        $monthTH = $thaiMonths[$englishMonth];
?>
<!DOCTYPE html>
<html lang="en">
...
<body id="page-top">
                ...
                <div class="container-fluid">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="container mt-5">
                            <h2 class="text-center mb-4">รายงานค่าเช่า <?= $monthTH ." " . $year+543?></h2>

                            <form action="report_payment_date.php" method="get" class="mb-5">
                                <div class="form-group">
                                    <label for="move_in_date">เลือกเดือน</label>
                                    <input type="month" class="form-control" id="date" value="<?= $month?>" name="date"
                                        required>
                                </div>
                            ...
                            <table class="table table-bordered bg-white" id="report_payment">
                                ...
                                <tbody>
                                    ...
                                    <?php
                                    $query = getData("SELECT *,DATE_FORMAT(bill_date, '%Y-%m') AS month_year FROM `tbl_room`
                                    ...
                                    WHERE DATE_FORMAT(bill_date, '%Y-%m') = '$month'
                                    ");
    ...
    <?php
            ...
            INNER JOIN tbl_room_type ON tbl_room_type.type_id = tbl_room.ref_type_id
            INNER JOIN tbl_user ON tbl_user.room_id = tbl_room.room_id
            INNER JOIN tbl_room_bill ON tbl_room_bill.user_id = tbl_user.user_id where tbl_room_bill.bill_status = 1 and DATE_FORMAT(tbl_room_bill.bill_date, '%Y-%m') = '$month'");

            $not_success = getData("SELECT DISTINCT * FROM `tbl_room`
            INNER JOIN tbl_room_type ON tbl_room_type.type_id = tbl_room.ref_type_id
            INNER JOIN tbl_user ON tbl_user.room_id = tbl_room.room_id
            INNER JOIN tbl_room_bill ON tbl_room_bill.user_id = tbl_user.user_id where tbl_room_bill.bill_status = 0 and DATE_FORMAT(tbl_room_bill.bill_date, '%Y-%m') = '$month'");
        ?>
